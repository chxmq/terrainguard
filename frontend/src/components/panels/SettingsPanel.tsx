          ...
          {[
            {
              label: "Slope",
              weight: "30%",
              desc: "Slope (Horn's method) — rate of elevation change; primary CFIT predictor"
            },
            {
              label: "TRI",
              weight: "30%",
              desc: "TRI (Riley's method) — mean elevation difference to neighbors; captures local roughness"
            },
            {
              label: "Curvature",
              weight: "20%",
              desc: "Curvature (Zevenbergen & Thorne) — rate of slope change; highlights ridge lines & valleys"
            },
            {
              label: "Elevation σ",
              weight: "20%",
              desc: "Elevation σ (Std Dev) — local height variation; captures macro-terrain relief"
            },
          ].map(({ label, weight, desc }) => (
            <div key={label} className="flex items-start gap-3 text-body-sm py-1" title={desc}>
              <span className="text-metric w-20 shrink-0 font-medium">{label}</span>
              <span className="text-metric-accent w-10 shrink-0 font-semibold">{weight}</span>
