...
export function CfitPanel() {
  ...
  if (error || !validation || !validation.summary || !validation.accidents) {
    return (
      <div className="rounded-md border border-amber-500/25 bg-amber-500/10 p-4">
        <h3 className="mb-2 text-sm font-semibold text-amber-200">Validation Report Unavailable</h3>
        <p className="text-xs leading-relaxed text-amber-100/90">
          {error && <span className="block mb-2 font-mono text-[11px] text-amber-300">{error}</span>}
          Please run the validation script to generate the report:
        </p>
        <div className="mt-3">
          <code className="rounded bg-secondary/80 px-2 py-1 font-mono text-[11px] text-foreground border border-border">
            python validate_ttci.py
          </code>
        </div>
        <p className="mt-3 text-[11px] text-muted-foreground">
          Run this command in the backend directory to compute predictive performance statistics against historical CFIT accident sites.
        </p>
      </div>
    );
  }
