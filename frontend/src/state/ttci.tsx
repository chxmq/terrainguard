    ...
    (async () => {
      ...
      try {
        ...
        if (i.ready && i.bounds) {
          ...
          try {
            const resp = await api.activateRegion({ south: 33.8, north: 34.5, west: 76.8, east: 77.8, source: "tiles" });
            setActiveRegion({ ...resp.bounds, zoom: resp.zoom, source: resp.source ?? "tiles" });
            const newInfo = await api.info();
            applyInfo(newInfo);
  ...
  const activate = useCallback(async (bbox: Bounds, source: string, options?: ActivateOptions) => {
    ...
    try {
      ...
      if (resp.is_synthetic) {
        toast("Real DEM download failed. Fell back to synthetic demonstration terrain.", "error");
